...
public class TcpClientHandler : IClientHandler
{
    ...
    private readonly CloneMine.ChatServer.Interfaces.IEncryptionService _encryptionService;
    private readonly CloneMine.Common.Interfaces.IRateLimiter _rateLimiter;

    public TcpClientHandler(IMessageHandler messageHandler, CloneMine.ChatServer.Interfaces.IEncryptionService encryptionService, CloneMine.Common.Interfaces.IRateLimiter rateLimiter)
    {
    ...
    public async Task HandleClientAsync(TcpClient client)
    {
        ...
        {
            Console.WriteLine($"[ChatServer] Rate limit exceeded for IP: {clientIp} - connection rejected");
            client.Close();
            return;
        }
        
        Console.WriteLine($"[ChatServer] Client connected: {remoteEndPoint}");
        ...
        try
        {
            ...
            while (client.Connected)
            {
                ...
                {
                    Console.WriteLine($"[ChatServer] Error processing message: {ex.Message}");
                }
            }
        }
        catch (Exception ex)
        {
            Console.WriteLine($"[ChatServer] Client error ({remoteEndPoint}): {ex.Message}");
        }
        finally
        {
            client.Close();
            Console.WriteLine($"[ChatServer] Client disconnected: {remoteEndPoint}");
        }
    }
}
